...
grid = fdtd.Grid(shape=(1.55e-4, 1.55e-4, 1), )

# objects
grid[-40:-20, 0:1.55e-4, 0] = fdtd.Object(permittivity=10**2)

# source
grid[22, 0.775e-4, 0] = fdtd.PointSource(period=1550e-9 / (3e8), name="source", power=100.0)
...
for i in range(100):
	grid[-80, 10*i:10*i+10, 0] = fdtd.LineDetector(name="detector"+str(i))
#grid[-80, 50:51, 0] = fdtd.LineDetector(name="detector")
...
#for i in detector_array:
	#for j in i:
		#arr.append(j[2])
print(arr)
plt.plot([10*x for x in range(len(arr))], arr)
plt.scatter([10*x for x in range(len(arr))], arr)
plt.show()
