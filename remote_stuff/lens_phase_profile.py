...
from matplotlib.pyplot import subplot, plot, xlabel, ylabel, legend, title, suptitle, show, ylim, figure, subplots_adjust
...
def main(wl=40):
	...
	if results:
		...
		plot([x for x in range(30, 30+5*len(phase_profile_emergent), 5)], [fit_func(0, *detector) - fit_func(xdata[0], *detector) for detector in popt], label="Phase profile curvature inverse")
		...
		scatter([x for x in range(30, 30+5*len(phase_profile_emergent), 5)], [fit_func(0, *detector) - fit_func(xdata[0], *detector) for detector in popt], c=[["blue", "orange", "green", "red"][x%4] for x in range(len(phase_profile_emergent))])
