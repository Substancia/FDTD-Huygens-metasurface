...
def lens_simulation():
	...
	# grid
	grid = fdtd.Grid(shape=(6100, 1400, 1), grid_spacing=77.5e-9)
	grid.save_simulation("R_5737_d_140")

	# objects
	lens_width = 140
	lens_radius = 5737
	x, y = arange(-600, 600, 1), arange(lens_radius - lens_width/2, lens_radius, 1)
	X, Y = meshgrid(x, y)
	lens_mask = X**2 + Y**2 <= lens_radius**2
	for j, col in enumerate(lens_mask.T):
		for i, val in enumerate(flip(col)):
			if val:
				grid[130+i:130+lens_width-i, j+100:j+101, 0] = fdtd.Object(permittivity=1.5**2, name=str(i)+","+str(j))
				break
