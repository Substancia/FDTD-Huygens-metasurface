...
for j, col in enumerate(lens_mask.T):
	for i, val in enumerate(flip(col)):
		if val:
			slice_thickness = (lens_width//2)-i%(lens_width//2)
			if slice_thickness < thickness:
				continue
			# biconvex
			...
			# planoconvex
			grid[30-thickness:30, j+10:j+11, 0] = fdtd.Object(permittivity=(1.5*slice_thickness/thickness)**2, name=str(i)+","+str(j))
			#grid[30-thickness:30, j+10:j+11, 0] = fdtd.Object(permittivity=(((1.5-1)*slice_thickness+1*lens_width)/thickness)**2, name=str(i)+","+str(j))
			break
...
if results:
	...
	for i, detector in enumerate(phase_profile_emergent):
		for val in detector:
			if val < 0:
				phase_profile_emergent[i] = [x+1 for x in phase_profile_emergent[i]]
				break
	...
	phase_profile_emergent[0] = [x+0.15 for x in (array(phase_profile_emergent[2])+array(phase_profile_emergent[3]))/2]
